package com.mike.dojooverflow.controllers;
...
import com.mike.dojooverflow.services.AppService;
...
public class HomeController {
	...
	public String Index(Model model) {
		model.addAttribute("questions", this.aService.getQuestions());
		return "index.jsp";
	}
	@GetMapping("/new")
	...
	public String Show(@PathVariable("id") Long id, @ModelAttribute("ans") Answer ans, Model model) {
		model.addAttribute("question", this.aService.getQuestion(id));
		return "show.jsp";
	}
	@PostMapping("/")
	public String Create(@Valid @ModelAttribute("newQuest") NewQuestion newQuest, 
		...
		if(result.hasErrors())
			return "new.jsp";
		this.aService.createQuestion(newQuest);
		return "redirect:/";
	}
	@PostMapping("/answers")
